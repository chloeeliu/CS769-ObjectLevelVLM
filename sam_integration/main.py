...
def main(args):
    ...
    if args.train:
        ...
        train_loader = DataLoader(dataset[:len(dataset)//4], 1, shuffle=False, num_workers=2, collate_fn=dataset.collate_fn)
            ...
            for i, batch in enumerate(tqdm(train_loader)):
                if config["use_sam"]:
                    ...
                    if type(original_segmentations) == list:
                        for n in range(len(original_segmentations)):
                            original_segmentations[n] = mask.decode(original_segmentations[n])
                    else:
                        original_segmentations = [mask.decode(original_segmentations)]

                    masks = []
                    for n in range(len(original_segmentations)):
                        original_mask_points = np.stack(np.nonzero(original_segmentations[n])[::-1], -1)

                        if config["sam_strategy"] == "mean":
                            sample_point = np.mean(original_mask_points, axis=0)
                        elif config["sam_strategy"] == "random":
                            sample_point = original_mask_points[np.random.choice(original_mask_points.shape[0])]

                        cv2_image = cv2.imread(batch["path_to_image"][n])
                        cv2_image = cv2.cvtColor(cv2_image, cv2.COLOR_BGR2RGB)
                        predictor.set_image(cv2_image)

                        if config["sam_multi"]:
                            mask_sam, _, _ = predictor.predict(
                                point_coords=np.array([sample_point]),
                                point_labels=np.array([1]),
                                multimask_output=True
                            )
                            errors = np.array([skimage.metrics.adapted_rand_error(original_segmentations[n], mask.astype(int))[0] for mask in mask_sam])
                            best_mask = mask_sam[np.argmin(errors)]
                        else:
                            mask_sam, _, _ = predictor.predict(
                                point_coords=np.array([sample_point]),
                                point_labels=np.array([1]),
                            ...
                            best_mask = mask_sam[0]

                        img = Image.open(batch["path_to_image"][n])
                        seg_width, seg_height = img.size
                        pooled_mask = skimage.measure.block_reduce(
                            best_mask,
                            block_size=(math.floor(seg_height / config["n_patches"]),
                                        math.floor(seg_width / config["n_patches"])),
                            func=np.max
                        )

                        result_height, result_width = pooled_mask.shape
                        # If the result is smaller than 16x16, pad it with zeros
                        if result_height < config["n_patches"] or result_width < config["n_patches"]:
                            pad_height = config["n_patches"] - result_height
                            pad_width = config["n_patches"] - result_width
                            pooled_mask = np.pad(pooled_mask, ((0, pad_height), (0, pad_width)), mode='constant')

                        if result_height > config["n_patches"] or result_width > config["n_patches"]:
                            pooled_mask = pooled_mask[:config["n_patches"], :config["n_patches"]]

                        masks.append(torch.BoolTensor(np.append(1, pooled_mask.flatten())).to(config["device"]))
                else:
                    masks = batch["vit_mask"]
