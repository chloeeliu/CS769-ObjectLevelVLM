...
class OLIVE(nn.Module):
    ...
    def prepare_embeddings(self, segmentations, images, sentences, labels=None, return_retrieved_info = False, b_num=0, cropped_images=[]):
    ...
    def prepare_input(self, segmentations, images, sentences, labels=None, return_retrieved_info = False, b_num=0, cropped_images=[]):
        if return_retrieved_info:
            full_text_input, object_embeddings, labels, image_features, prompts, retrieved_masks, retrieved_images = (
                self.prepare_embeddings(segmentations, images, sentences, labels, return_retrieved_info, b_num, cropped_images))

            # Replace obj tokens with their object vector representation
            final_input, label_input, attention_mask = self.embed_with_special_tokens(full_text_input,
                                                                                      object_embeddings, labels=labels,
                                                                                      image_features=image_features)
            return final_input, label_input, attention_mask, prompts, retrieved_masks, retrieved_images
        else:
            full_text_input, object_embeddings, labels, image_features = (
                self.prepare_embeddings(segmentations, images, sentences, labels, return_retrieved_info, b_num, cropped_images))
            # Replace obj tokens with their object vector representation
            final_input, label_input, attention_mask = self.embed_with_special_tokens(full_text_input,
                                                                                      object_embeddings, labels=labels,
