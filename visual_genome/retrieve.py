...
def get_object_feature_visual_genome(image, mask, processor, model, config):
    images = [image]
    inputs = processor(images=images, return_tensors="pt").to(config["device"])
    image_forward_outs = model(inputs['pixel_values'], output_hidden_states=True)
    mask = torch.BoolTensor(mask[1:]).to(config["device"])
...
def main(args):
    ...
    if args.train:
        ...
        retrieval_index = []
        object_position = []
        root = '/u/h/k/hkhader/research/datasets/outputs/olive'
        retrieval_path = f'{root}/retrieval/{config["task"]}/retrieval_set_{config["examples_per_class"]}_{cropped}{config["vision_encoder"].split("/")[-1]}.pkl'
        ...
        if "additional_retrieval_examples" in config:
            ...
            dataset = VisualGenomeDataset(config, split="train", patch_size=config['n_patches'], images_num=None, entries_num=400000) # pre experiment is with retrieval 100
            train_loader = DataLoader(dataset, config["batch_size"], shuffle=False, num_workers=2, collate_fn=dataset.collate_fn)   
        ...
        if config['task'] == 'relation_prediction':
            for index, object_entry in enumerate(tqdm(dataset.retrieval_entries)):
                image = object_entry["path_to_image"]
                mask = object_entry["vit_mask"]
                answer = object_entry["answer"]
                
                object_feature = get_object_feature_visual_genome(image, mask, processor, model, config)
            ...
            for b_num, batch in enumerate(tqdm(train_loader)):
                ...
                #     answer1, relation, answer2 = batch['answer'][0].split("|||")
                #     image1, image2 = batch['path_to_image'][0]
                #     mask1, mask2 =  batch['vit_mask'][0]
                    
                    
                #     def add_object_features(image, mask, answer, position):
                #         object_feature = get_object_feature_visual_genome(image, mask, processor, model, config)
                #         if object_feature == None:
                #             return
                #         if len(torch.nonzero(torch.isnan(object_feature.view(-1)))) > 0:
                ...
                #         object_position.append(position)
                    
                #     add_object_features(image1, mask1, answer1, 0)
                #     add_object_features(image2, mask2, answer2, 1)
        ...
        retrieval_set['object_position'] = object_position
