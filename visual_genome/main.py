...
def main(args):
    ...
    if args.train:
        ...
        if config['task'] == 'object_classification':
            ...
            dataset = VRPDataset("train", n_patches=config['n_patches'], use_object_annotations=config['use_object_annotations'], long_answers=True)
            #dataset = VRPDataset("train", n_patches=config['n_patches'], use_object_annotations=config['use_object_annotations'])
            train_loader = DataLoader(dataset, config["batch_size"], shuffle=False, num_workers=2, collate_fn=dataset.collate_fn)
        elif config['task'] == 'classification_and_detailed_image_captioning':
            ...
            dataset2 = VRPDataset("train", n_patches=config['n_patches'], use_object_annotations=config['use_object_annotations'], long_answers=True)
            
            random.shuffle(dataset1.entries)
            random.shuffle(dataset2.entries)
            
            dataset2.entries = dataset1.entries + dataset2.entries
            dataset = dataset2
            ...
            dataset = VisualGenomeDataset(config, split="train", patch_size=config['n_patches'], images_num=None, entries_num=400000) # images_num=100 was used for current tesnting. 
            train_loader = DataLoader(dataset, config["batch_size"], shuffle=False, num_workers=8, collate_fn=dataset.collate_fn)
        ...
        if args.resume:
            model.load()
            checkpoint = torch.load(os.path.join(model._get_save_path(), 'checkpoint.pth'), map_location=config["device"])
        ...
        if config["load_pretrained_model"]:
            model.load_from_pretrained(config["pretrained_path"])
        ...
        for epoch in range(start_epoch, config["n_epochs"]):
            ...
            for b_num, batch in enumerate(tqdm(train_loader)):
                ...
                if type(masks) == list: # it's always a list!!!
        ...
        if config['task'] == 'object_classification':
            ...
            dataset = VisualGenomeDataset(config, split="val", patch_size=config['n_patches'], images_num=None, entries_num=60000)#images_num=None, entries_num=60000)
            #train_loader = DataLoader(dataset, config["batch_size"], shuffle=False, num_workers=2, collate_fn=dataset.collate_fn)
        elif config['task'] == "open_vocab_object_classification":
            dataset = LVISDataset(config, split="val", n_patches=config['n_patches'])
        elif config['task'] == 'image_captioning':
            dataset = VRPDataset("val", n_patches=config['n_patches'])
        elif config['task'] == 'classification_and_detailed_image_captioning':
            dataset = VRPDataset("val", n_patches=config['n_patches'])
        elif config['task'] == "refCOCOg":
        ...
        if config["use_retrieval"]:
            ...
            if config['task'] == 'object_classification':
                ...
                retrieval_dataset = VisualGenomeDataset(config, split="train", patch_size=config['n_patches'], images_num=None, entries_num=400000)
        ...
        ROOT_SAVE_PATH = "/u/h/k/hkhader/research/datasets/outputs/olive"
        OUTPUT_SAVE_PATH = os.path.join(ROOT_SAVE_PATH, "outputs", config["task"], config["llm_model"].split("/")[-1] + "_" + model._get_save_path(load_raw=True).split("/")[-1] + ".pkl")
        os.makedirs(os.path.join(ROOT_SAVE_PATH, "outputs", config["task"]), exist_ok=True)
        ...
        with torch.no_grad():
            for i, batch in enumerate(tqdm(train_loader)):    
                ...
                if i == 50:
                    directory, filename = os.path.split(OUTPUT_SAVE_PATH)
                    new_filename = "sample_responses" + filename
                    new_save_path = os.path.join(directory, new_filename)
                    
                    with open(new_save_path, "wb") as f:
                        print(f"Saved model outputs to {new_save_path}")
        ...
        # with open(OUTPUT_SAVE_PATH, "wb") as f:
        #     print(f"second Saved model outputs to {OUTPUT_SAVE_PATH}")
        #     pickle.dump(responses, f)
...
if __name__ == "__main__":
    ...
    # # Set the cache directory for Hugging Face models and datasets
