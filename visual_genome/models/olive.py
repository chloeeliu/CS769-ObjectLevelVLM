...
class OLIVE(nn.Module):
    ...
    def __init__(self, config, retrieval_fn = None):
        super().__init__()
        temp_dir = "/nobackup3/hkhader/temp_dir"
        self.config = config
        ...
        self.temp_dir = "/nobackup3/hkhader/temp_dir"
        ...
        if "llama" in base_model:
            ...
            self.tokenizer = LlamaTokenizer.from_pretrained(base_model, cache_dir=self.temp_dir)
    ...
    def prepare_input(self, segmentations, images, sentences, labels=None, return_retrieved_info = False, b_num=0, cropped_images=[]):
        ...
        if self.config["use_retrieval"] and len(segmentations) > 0:
            if self.config["crop_image"]:
                ...
                prompts1, retrieved_masks1, retrieved_images1 = self.get_retrieval_prompt(segmentations, images, b_num=b_num)
                
                # hacky implementation: get_retrieval_prompt now only do it for first make
                segmentations2 = [segmentations_batch[1:2] for segmentations_batch in segmentations] # get second mask
                prompts2, retrieved_masks2, retrieved_images2 = self.get_retrieval_prompt(segmentations2, images, b_num=b_num)
                
                def pairwise_combination():
                    # Ensure the lists are of the same length
                    assert len(prompts1) == len(prompts2), "prompts lists must have the same length"
                    assert len(retrieved_masks1) == len(retrieved_masks2), "retrieved_masks lists must have the same length"
                    assert len(retrieved_images1) == len(retrieved_images2), "retrieved_images lists must have the same length"
                    
                    # Pairwise string concatenation for prompts
                    prompts = ["For first object " +  p1 + " For second object " + p2 for p1, p2 in zip(prompts1, prompts2)]
                    
                    # Pairwise tensor concatenation for retrieved masks
                    retrieved_masks = [torch.cat((mask1, mask2), dim=0) for mask1, mask2 in zip(retrieved_masks1, retrieved_masks2)]
                    
                    # Pairwise tensor concatenation for retrieved images
                    retrieved_images = [
                    img_list1 + img_list2 for img_list1, img_list2 in zip(retrieved_images1, retrieved_images2)
                    ]
                    
                    return prompts, retrieved_masks, retrieved_images
                
                prompts, retrieved_masks, retrieved_images = pairwise_combination()
    ...
    def get_retrieval_prompt(self, mask, images, b_num = 0, cropped_images = []):
        ...
        if len(cropped_images) > 0:
            ...
            if type(mask) == list:  # it's always a list!!!
                mask_copy = mask.copy()
    ...
    def _get_save_path(self, load_raw=False):
        ...
        SAVE_PATH = ""
        SAVE_PATH += self.config["model_save_load_name_prefix"] if self.config["model_save_load_name_prefix"] else ""
        if self.config["freeze_llm"]:
    ...
    #     self.llama_model = PeftModel.from_pretrained(self.llama_model, SAVE_PATH, cache_dir=self.temp_dir)
    
    def load(self):
        SAVE_PATH = self._get_save_path()
        print(f"The save path is: {SAVE_PATH}")
        
        # Load the object encoder with map_location and then move it to the device
        object_encoder_path = os.path.join(SAVE_PATH, "llama_2_7b_adapter_finetuned")
        self.object_encoder.load_state_dict(torch.load(object_encoder_path, map_location=self.config["device"]))
        self.object_encoder.to(self.config["device"])  # Ensure it's on the correct device
        print(f"Loaded Object Encoder checkpoint from {SAVE_PATH} to {self.config['device']}")
        
        if self.config["freeze_llm"]:
            return
        
        # Load the LLM model with map_location and move to the specified device
        if "gpt2" in self.config["llm_model"]:
            self.llama_model = AutoModelForCausalLM.from_pretrained(
                SAVE_PATH
            )
            self.llama_model.to(self.config["device"])  # Explicitly move it to the correct device
            print(f"Loaded GPT-2 model to {self.config['device']}")
            return
        
        # For non-GPT2 models, apply map_location and move to the specified device
        ...
        self.llama_model.to(self.config["device"])  # Ensure it's on the correct device
        print(f"Loaded Llama model to {self.config['device']}")

        
    
    def load_from_pretrained(self, model_dir):
        self.object_encoder.load_state_dict(torch.load(os.path.join(model_dir, "llama_2_7b_adapter_finetuned")))
        print(f"Loaded Object Encoder checkpoint from {model_dir}")
        if self.config["freeze_llm"]:
            return
        if "gpt2" in self.config["llm_model"]:
            self.llama_model = AutoModelForCausalLM.from_pretrained(
                model_dir).to(self.config["device"])
            return
        
        self.llama_model = PeftModel.from_pretrained(self.llama_model, model_dir, cache_dir=self.temp_dir)
