...
def compute_analogy(images_1, images_2, images_3, backbone, use_retrieval, freeze_llm, chat_history):
    global model
    global old_config
    ...
    if config['use_retrieval']:
        ...
        full_text_input_3, object_embeddings_3, labels_3, image_features_3, prompts_3, retrieved_masks_3, retrieved_images_3 = (
            generate_encoding(question, images_3, output_height, output_width))
        ...
        retrieval_images = [Image.open(retrieved_images_1[0][x]) for x in range(len(retrieved_images_1[0]))]
        retrieval_images.extend([Image.open(retrieved_images_2[0][x]) for x in range(len(retrieved_images_2[0]))])
        retrieval_images.extend([Image.open(retrieved_images_3[0][x]) for x in range(len(retrieved_images_3[0]))])
...
with gr.Blocks(title="Olive", theme=gr.themes.Base()).queue() as demo:
    ...
    submit_btn.click(fn=compute_analogy,
                     inputs=[gallery_1, gallery_2, gallery_3, backbone, use_retrieval, freeze_llm, chatbot],
