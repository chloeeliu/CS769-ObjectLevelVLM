...
def main(args):
    ...
    if args.train:
        ...
        if config['task'] == 'object_classification':
            ...
            # Shuffle the indices and select 10%
            random.shuffle(indices)
            subset_indices = indices[:subset_size]
            
            # Create a sampler for the subset
            sampler = SubsetRandomSampler(subset_indices)
            train_loader = DataLoader(dataset, batch_size = config["batch_size"], sampler=sampler, shuffle=False, num_workers=2, collate_fn=dataset.collate_fn)
        ...
        OUTPUT_SAVE_PATH = os.path.join(
            "outputs",
            config["task"],
            os.path.basename(config["llm_model"]) + "_" + os.path.basename(model._get_save_path(load_raw=True)) + ".pkl"
        )
        ...
        # Here, we calculate 10% of the dataset size
        num_samples = len(dataset)
        test_size = int(1 * num_samples)
        indices = list(range(num_samples))
        random.shuffle(indices)
        indices = indices[:test_size]

        # Create a DataLoader that only loads the subset
        sampler = SubsetRandomSampler(indices)

        train_loader = DataLoader(dataset, 1, sampler=sampler, shuffle=False, num_workers=2, collate_fn=dataset.collate_fn) # add sample = sampler
