...
class OLIVE(nn.Module):
    ...
    def __init__(self, config, retrieval_fn = None):
        ...
        # Freeze the vision encoder within ObjectEncoder
        self.object_encoder.model.requires_grad_(False)
        # Ensure the attention layer is trainable
        for param in self.object_encoder.attention_fc.parameters():
    ...
    def prepare_input(self, segmentations, images, sentences, labels=None, return_retrieved_info=False, b_num=0, cropped_images=[]):
        ...
        if len(segmentations) > 0:
            for i in range(len(segmentations)):
                # Prepare images and get features from the vision model
                inputs = self.object_encoder.processor(images=images[i], return_tensors="pt").to(self.config["device"])
                transformer_output = self.object_encoder.model(**inputs).last_hidden_state  # Shape: (batch_size, seq_len, hidden_dim)
                ...
                # Prepare mask_input
                mask_input = segmentations[i]  # Shape: (num_objects, seq_len)
                if len(mask_input.shape) == 1:
                    mask_input = mask_input.unsqueeze(0).to(self.config["device"])

                # If user query involves multiple objects, duplicate image feature if necessary
                if transformer_output.shape[0] != mask_input.shape[0]:
                    if transformer_output.shape[0] == 1:
                ...
                # Call the ObjectEncoder with mask_input and transformer_output
            ...
            inputs = self.object_encoder.processor(images=images[0], return_tensors="pt").to(self.config["device"])
    ...
    def load(self):
        ...
        # Determine whether to load PEFT adapters
        if not self.config["freeze_llm"] and "gpt2" not in self.config["llm_model"]:
            # Load the base model
            self.llama_model = AutoModelForCausalLM.from_pretrained(
            ...
            self.llama_model = AutoModelForCausalLM.from_pretrained(SAVE_PATH).to(self.config["device"])
